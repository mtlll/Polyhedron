...
int playsound(int n, const vec *loc, entities::classes::CoreEntity *ent, int flags, int loops, int fade, int chanid, int radius, int expire)
{
    ...
    if(chanid < 0)
    {
        ...
        if(shared::network::ftsClient.totalMilliseconds == lastsoundmillis) soundsatonce++; else soundsatonce = 1;
        lastsoundmillis = shared::network::ftsClient.totalMilliseconds;
