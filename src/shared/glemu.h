namespace gle
{
    ...
    #define GLE_INITATTRIB(name, index, suffix, type) \
        static inline void name##suffix(type x) { glCheckError(glVertexAttrib1##suffix##_(index, x)); } \
        static inline void name##suffix(type x, type y) { glCheckError(glVertexAttrib2##suffix##_(index, x, y)); } \
        static inline void name##suffix(type x, type y, type z) { glCheckError(glVertexAttrib3##suffix##_(index, x, y, z)); } \
    ...
    static inline void normal(const vec &v) { glCheckError(glVertexAttrib4f_(ATTRIB_NORMAL, v.x, v.y, v.z, 0.0f)); }
    static inline void tangent(float x, float y, float z, float w = 1.0f) { glCheckError(glVertexAttrib4f_(ATTRIB_TANGENT, x, y, z, w)); }
    static inline void tangent(const vec &v, float w = 1.0f) { glCheckError(glVertexAttrib4f_(ATTRIB_TANGENT, v.x, v.y, v.z, w)); }
    static inline void tangent(const vec4 &v) { glCheckError(glVertexAttrib4fv_(ATTRIB_TANGENT, v.v)); }

    #define GLE_ATTRIBPOINTER(name, index, defaultnormalized, defaultsize, defaulttype, prepare) \
        static inline void enable##name() { prepare; glCheckError(glEnableVertexAttribArray_(index)); } \
        static inline void disable##name() { glCheckError(glDisableVertexAttribArray_(index)); } \
        static inline void name##pointer(int stride, const void *data, GLenum type = defaulttype, int size = defaultsize, GLenum normalized = defaultnormalized) { \
            prepare; \
            glCheckError(glVertexAttribPointer_(index, size, type, normalized, stride, data)); \
        }

    static inline void enableattrib(int index) { disable(); glCheckError(glEnableVertexAttribArray_(index)); }
    static inline void disableattrib(int index) { glCheckError(glDisableVertexAttribArray_(index)); }
